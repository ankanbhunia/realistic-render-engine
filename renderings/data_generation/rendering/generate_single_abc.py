...
def main():
    ...
    # floor_pbr = np.random.choice(pbr_files) # 'Carpet013'#
    ...
    if args.dataset_type == 'ABC':
        ...
        if os.path.isfile(args.pose_file_name):
    ...
    # Step 4: Select all faces in the object
    ...
    for multiplier_factor in range(1, 4):
    ...
    radius_max = (abs(max_bounds[0]-min_bounds[0])+
                    abs(max_bounds[1]-min_bounds[1])+
                    abs(max_bounds[2]-min_bounds[2]))/3#max(abs(min_bounds[0]), abs(min_bounds[1]), abs(max_bounds[0]), abs(max_bounds[1]))
    height_max = max_bounds[2]
    print(radius_max)

    radius = random.uniform(radius_max*1, radius_max*2)
    height = random.uniform(radius_max*1, radius_max*2) #+0.4
    ...
    print(angles)
    radii = random_wave_function(min = radius, max = radius+random.uniform(0, 0.05), n_frames =  N_FRAMES) #np.random.uniform(low=radius-0.2, high=radius+0.2, size=N_FRAMES)
    x_cam_co = radii*np.cos(angles)
    y_cam_co = radii*np.sin(angles)
    z_cam_co = random_wave_function(min = height, max = height+random.uniform(0, 0.05), n_frames = N_FRAMES) #np.random.uniform(height-0.1, height+0.1, N_FRAMES)
    ...
    print (opt_means)
    track_to_points[:,0:2] = track_to_points[:,0:2] + \
    ...
    try:
        ...
        bpy.ops.export_scene.obj(
            filepath=os.path.join(output_dirpath, 'transformed.obj'),
            use_selection=True,
